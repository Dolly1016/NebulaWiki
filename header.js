...
function getAssignablesMenu(assignablesMap){
    tryGetAssignableMenu = function(category, display){
        if(assignablesMap.has(category)){
            ...
            return '<li class="nav-item dropdown"><a class="nav-link dropdown-toggle" role="button" data-bs-toggle="dropdown" aria-expanded="false">' + display + '</a><ul class="dropdown-menu scrollable-menu" role="menu" style="height: auto;max-height: 400px;overflow-x: hidden;">' + inner + '</ul></li>';
        }
        return '';
    }

    return tryGetAssignableMenu('crewmates', 'クルーメイト役職') + 
    tryGetAssignableMenu('impostors', 'インポスター役職') + 
    tryGetAssignableMenu('neutral', '第三陣営役職') + 
    tryGetAssignableMenu('modifiers', 'モディファイア') + 
    tryGetAssignableMenu('ghostRoles', '幽霊役職') //+ 
//    tryGetAssignableMenu('commonPerks', '共通パーク') + 
//    tryGetAssignableMenu('evilPerks', '人外専用パーク')
}
